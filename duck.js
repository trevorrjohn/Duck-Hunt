...
function mainMenu() {
  if(document.cookie != "" && navigator.onLine) {
    ...
    while(info.length != 0){
      var user = info.pop();
      var scorecook = parseInt(info.pop());
      console.log("cookie being sent:" + scorecook + " " + user);
      $.post("http://score-app.heroku.com/api/v1/scores", {game: "Duck-Hunt", score: scorecook*10, username: user});
      document.cookie="";
    }
  }
  if(played){
